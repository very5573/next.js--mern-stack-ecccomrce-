...
export default function MyProfile() {
  // ✅ SAFE & STABLE SELECTOR (NO WARNING)
  ...
  // ⏳ wait until auth check is done (on reload)
  if (!authChecked) {
    return (
            ...
            {user.avatar ? (
              <>
                <Avatar
                  src={user.avatar}
                  alt={user.name}
                  className="w-40 h-40 cursor-pointer shadow-xl hover:scale-105 transition-transform duration-300"
